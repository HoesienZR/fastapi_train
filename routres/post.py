...
async def get_posts(db_session : Annotated[AsyncSession,Depends(get_db)])->post_output.PostsOutPut:
    """this is return all posts"""
    posts = await BlogOperation(db_session=db_session).get_posts()
    return posts
...
@router.put('/update_post/') #TODO: this is need test
